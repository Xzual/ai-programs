  ...
  return (
    <div className="edith-desktop-titlebar flex h-9 shrink-0 items-center justify-between border-b border-white/10 bg-black/34 px-2 text-[11px] text-slate-400">
      <button
        ...
        title={status.tauri ? 'E.D.I.T.H. penceresini sürükle' : 'Tarayıcı önizleme modu'}
      >
      ...
      <div className="flex items-center gap-1">
        ...
        <button type="button" onClick={() => void runWindowCommand('minimize_window')} disabled={!status.tauri} className="rounded-md p-1.5 text-slate-400 hover:bg-white/10 hover:text-slate-100 disabled:opacity-35" title="Küçült">
          <Minus className="h-3.5 w-3.5" />
        </button>
        <button type="button" onClick={() => void runWindowCommand('toggle_maximize')} disabled={!status.tauri} className="rounded-md p-1.5 text-slate-400 hover:bg-white/10 hover:text-slate-100 disabled:opacity-35" title="Büyüt">
          <Square className="h-3.5 w-3.5" />
        </button>
        <button type="button" onClick={() => void runWindowCommand('toggle_fullscreen')} disabled={!status.tauri} className="rounded-md p-1.5 text-slate-400 hover:bg-white/10 hover:text-slate-100 disabled:opacity-35" title="Tam ekranı aç/kapat">
          <Maximize2 className="h-3.5 w-3.5" />
        </button>
        <button type="button" onClick={() => void runWindowCommand('close_window')} disabled={!status.tauri} className="rounded-md p-1.5 text-slate-400 hover:bg-red-500/20 hover:text-red-100 disabled:opacity-35" title="Kapat">
